function initialiserDonnees() {
  // Déclaration du JSON contenant les données de pollution
  let data = {
    polluant: "CO2",
  ...
  // Construction du contenu HTML pour chaque pays
  let html = "";
  for (let i = 0; i < data.pays.length; i++) {
    let pays = data.pays[i];
    html += `
      <div class="pays">
        <img src="https://flagcdn.com/48x36/${pays.code}.png" alt="Drapeau de ${pays.nom}">
        <strong>${pays.nom}</strong><br>
        Émissions : ${pays.valeur} ${data.unite}<br>
        Part mondiale : ${pays.pourcentage}%
      </div>
    `;
  }

  // Injection du HTML dans la page
  document.getElementById("listePays").innerHTML = html;
}
